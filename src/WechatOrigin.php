...
class WechatOrigin
{
    ...
    // 新增
    const EVENT_CARD_USER_VIEW = 'user_view_card';        // 会员卡 - 用户点击会员卡 zbn
    const CARD_MEMBERCARD_ACTIVATETEMPIFO_GET = '/card/membercard/activatetempinfo/get?'; // 根据activate_ticket获取到用户填写的信息
    const CARD_MEMBERCARD_ACTIVATEUSERFORM_SET = '/card/membercard/activateuserform/set?'; // 设置开卡字段
    ...
    //小程序代码模版库
    const GET_TEMPLATE_DRAFT_LIST = '/gettemplatedraftlist?'; //获取草稿箱内的所有临时代码草稿
    const GET_TEMPLATE_LIST = '/gettemplatelist?'; //获取代码模版库中的所有小程序代码模版
    const ADD_TO_TEMPLATE = '/addtotemplate?'; //将草稿箱的草稿选为小程序代码模版
    const DELETE_TEMPLATE = '/deletetemplate?'; //删除指定小程序代码模版
...
//            Log::record('[postStr]' . var_export($postStr, true), 'wechat');
            $array = (array)simplexml_load_string($postStr, 'SimpleXMLElement', LIBXML_NOCDATA);
//            Log::record('[array]' . var_export($array, true), 'wechat');
            $this->encrypt_type = isset($_GET["encrypt_type"]) ? $_GET["encrypt_type"] : '';
//            Log::record('[encrypt_type]' . var_export($this->encrypt_type, true), 'wechat');
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    protected function setCache($cachename, $value, $expired)
    {
        ...
        return Cache::store('default')->set($cachename, $value, $expired);
    }
    ...
    protected function getCache($cachename)
    {
        ...
        return Cache::store('default')->get($cachename);
    }
    ...
    protected function removeCache($cachename)
    {
        ...
        return Cache::store('default')->rm($cachename);
    }
...
//        $arrdata = array("api_ticket" => $this->api_ticket, "app_id" => $this->appid, "card_id" => $card_id, "code" => $code, "card_type" => $card_type, "location_id" => $location_id, "timestamp" => $timestamp, "noncestr" => $noncestr);
        $arrdata = array("api_ticket" => $this->api_ticket, "card_id" => $card_id, "code" => $code, "card_type" => $card_type, "location_id" => $location_id, "timestamp" => $timestamp, "noncestr" => $noncestr);
     ...
     * 注意：unionid字段 只有在用户将公众号绑定到微信开放平台账号后，才会出现。建议调用前用isset()检测一下
     * 注意：该接口只有再授权scope方式为snsapi_userinfo才可以调用，否在返回48001
     */
    ...
    /**
     * 激活/绑定会员卡
     * @param array $data 具体结构请参看卡券开发文档(6.1.1 激活/绑定会员卡)章节
     * @return boolean
     */
    ...
    /**
     * 会员卡 - 根据activate_ticket获取到用户填写的信息
     * @param string $activateTicket activate_ticket数据
     * @return array|boolean 返回数据
     */
    public function getActivateTicketInfo($activateTicket)
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_MEMBERCARD_ACTIVATETEMPIFO_GET . 'access_token=' . $this->access_token, self::json_encode(['activate_ticket' => $activateTicket]));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return array|boolean 返回数据
     */
    public function setActivateuserform($data)
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_MEMBERCARD_ACTIVATEUSERFORM_SET . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    {
        if (!$this->component_access_token && !$this->checkComponentAuth()) return false;
        $result = $this->http_get(self::WXA_URL_PREFIX . self::GET_TEMPLATE_DRAFT_LIST . 'access_token=' . $this->component_access_token);
        if ($result) {
            $json = json_decode($result, true);
    ...
    {
        if (!$this->component_access_token && !$this->checkComponentAuth()) return false;
        $result = $this->http_get(self::WXA_URL_PREFIX . self::GET_TEMPLATE_LIST . 'access_token=' . $this->component_access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || $json['errcode'] != 0) {
                $this->errCode = $json['errcode'];
    ...
    /**
     * 将草稿箱的草稿选为小程序代码模版
     * @param  int $draftId 草稿ID，本字段可通过“ 获取草稿箱内的所有临时代码草稿 ”接口获得
     * @return bool|string
     */
    public function addToTemplate($draftId)
    {
        ...
        $result = $this->http_post(self::WXA_URL_PREFIX . self::ADD_TO_TEMPLATE . 'access_token=' . $this->component_access_token, self::json_encode($params));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || $json['errcode'] != 0) {
                $this->errCode = $json['errcode'];
    ...
    public function deleteTemplate($templateId)
    {
        ...
        $result = $this->http_post(self::WXA_URL_PREFIX . self::DELETE_TEMPLATE . 'access_token=' . $this->component_access_token, self::json_encode($params));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || $json['errcode'] != 0) {
                $this->errCode = $json['errcode'];
    ...
    /**
     * 使用code 换取 session_key  (微信登录小程序时)
     * @param  string $appid 小程序的AppID
     * @param  string $jsCode 登录时获取的 code
     * @return bool|string
     */
    public function jscode2session($appid, $jsCode)
    {
        if (!$this->component_access_token && !$this->checkComponentAuth()) return false;
        $result = $this->http_get('https://api.weixin.qq.com/sns/component/jscode2session?appid=' . $appid . '&js_code=' . $jsCode . '&grant_type=authorization_code&component_appid=' . $this->appid . '&component_access_token=' . $this->component_access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || $json['errcode'] != 0) {
                $this->errCode = $json['errcode'];
